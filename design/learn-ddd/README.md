...
#### よく DDD の文脈で会話していると「ドメイン知識が漏れている」という表現を目にすることがあります。これは一体どういう意味なのか説明するため、ドメイン知識が漏れているサンプルコードを作成してください

ドメイン知識が漏れているとは、ドメイン固有のロジックや知識が不適切な層に散らばっている状態を指す。

以下のコードがインフラ層にあるとします。本来価格計算ロジックは、ドメイン層に属するべきでインフラ層はドメインオブジェクトの永続化に専念するべきです。

```ts
function calculateDiscountSubtotal(order: Order): number {
  if (order.total > 100) {
...
### DDD に関するクイズを作成してください

問題: ドメインサービスの特徴はなんですか。
回答: エンティティやバリューオブジェクトに属さない、ビジネスロジックを実装する

問題: ファクトリーパターンを利用すべきなのはどのような状況でしょうか。
回答: エンティティやバリューオブジェクトの作成時に複雑な初期化ロジックが必要な場合

問題: イベントストーミング（Event Storming）を行う目的はなんですか。
回答: ドメインエキスパートと開発者の間でユビキタス言語を共有し、ドメインの理解を深める
