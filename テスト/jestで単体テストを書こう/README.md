...
1. OSSライブラリを使って開発をしている場合テストケースから細かい挙動(エッジケース)を読み解くことができそう
2. ほとんどUnitテストだが、稀にe2eテストがあったり、UI周りのテストがありもする。[Playwright](https://playwright.dev/)がよく使われている印象。
3. testsディレクトリがルートにあるケースもあるが、各モジュールの下にあったりもする。プロジェクトの大きさによるのかも。